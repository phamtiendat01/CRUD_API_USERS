...
const ROLE_OPTIONS: (Role | "all")[] = ["all", "user", "admin"];
...
export default function UsersPage() {
  ...
  const statusCount = useMemo(() => {
    const m: Record<string, number> = { all: rows.length, active: 0, inactive: 0, banned: 0 };
  ...
  function resetFilters() {
    setQ(""); setRole("all"); setStatus("all");
  }
  ...
  return (
    <AppShell>
      ...
      {/* Filter hàng ngang + 2 icon bên phải (cùng hàng) */}
      <div className="card p-3 mb-4">
        <div className="grid grid-cols-1 md:grid-cols-5 gap-3 items-center">
          {/* search chiếm 2 cột để dài như ảnh */}
          <input
          ...
          {/* Nút Reset + Thêm nằm cùng hàng, canh phải */}
          <div className="flex justify-end gap-2">
            <button className={iconBtn} aria-label="Reset" title="Reset" onClick={resetFilters}>
              <IconReset className="h-5 w-5" />
            </button>
            <button className={iconBtnPrimary} aria-label="Thêm người dùng" title="Thêm người dùng"
      ...
      <div className="card overflow-hidden">
        <table className="w-full table-auto text-sm">
          ...
          <tbody className="divide-y divide-gray-100">
            {loading && (
              <tr><td colSpan={8} className="px-4 py-8 text-center text-gray-500">Đang tải dữ liệu…</td></tr>
            )}
              ...
              <tr key={u.id} className="hover:bg-indigo-50/40 transition">
                ...
                <td className="px-4 py-3">
                  <span className={`px-2.5 py-1 rounded-full text-xs
                    ${u.status==="active"   ? "bg-emerald-50 text-emerald-700 ring-1 ring-emerald-200" :
                      u.status==="inactive" ? "bg-amber-50 text-amber-700 ring-1 ring-amber-200" :
            ...
            {!loading && data.length === 0 && (
              <tr><td colSpan={8} className="px-4 py-10 text-center text-gray-500">Không có dữ liệu</td></tr>
            )}
          </tbody>
        </table>
      </div>
      ...
      <Modal open={!!delUser} onClose={() => setDelUser(null)} title="Xóa người dùng">
        <p>Bạn chắc chắn muốn xóa <b>{delUser?.username}</b>?</p>
        <div className="mt-5 flex justify-end gap-2">
          ...
          <button
            className="h-9 px-3 rounded-lg bg-rose-600 text-white hover:bg-rose-700"
